...
function Toggle({ defaultOn = false }: { defaultOn?: boolean }) {
  ...
  return (
    <button
      onClick={() => setOn(!on)}
      className={`w-[38px] h-5 rounded-[10px] relative transition-colors shrink-0 ${on ? "bg-primary" : "bg-border"}`}
    >
  ...
  return (
    <div>
      <div className="mb-4">
        ...
        <p className="text-xs text-muted-foreground mt-[2px]">Configure your shop preferences</p>
      </div>
