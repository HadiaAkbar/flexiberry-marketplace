    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-text3 mt-[2px]">Track and manage all customer orders</p>
        </div>
      </div>
      ...
      <div className="bg-card border border-border rounded-lg shadow-card overflow-hidden">
        ...
        <div className="flex items-center justify-between p-[14px_16px] border-b border-border">
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-[7px] bg-background border border-border rounded-[7px] px-[11px] h-8 w-[200px]">
              <Search className="w-3.5 h-3.5 text-text3" />
              <input type="text" placeholder="Search orders..." className="border-none outline-none bg-transparent text-foreground text-xs font-sans w-full placeholder:text-text3" />
            </div>
            {statusTabs.map(t => (
              <button
                key={t}
                ...
                className={`px-3 py-[6px] rounded-lg text-xs font-semibold border transition-all ${
                  activeTab === t ? "bg-primary text-primary-foreground border-primary" : "bg-card text-muted-foreground border-border hover:border-border2"
                }`}
              >
                {t}
              </button>
          ...
          <span className="text-xs text-text3">847 total orders</span>
        </div>
          ...
          <thead className="bg-background">
            <tr>
              {["Order ID", "Customer", "Product", "Amount", "Plan", "Monthly", "Progress", "Status", "Actions"].map(h => (
                <th key={h} className="px-4 py-[10px] text-left text-[11px] font-bold text-text3 uppercase tracking-[0.7px] whitespace-nowrap">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {filtered.map((o) => (
              <tr key={o.id} className="hover:bg-foreground/[0.014]">
                ...
                <td className="px-4 py-3 border-t border-border">
                  <div className="flex items-center gap-2">
                    <div className="w-7 h-7 rounded-full bg-gradient-to-br from-primary to-fb-blue flex items-center justify-center text-[10px] font-bold text-primary-foreground shrink-0">
                      {o.initials}
                    </div>
                    <span className="text-[13px] font-semibold text-foreground">{o.customer}</span>
                  </div>
                </td>
                <td className="px-4 py-3 text-[13px] text-muted-foreground border-t border-border">{o.product}</td>
                ...
                <td className="px-4 py-3 border-t border-border w-[100px]">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-text3 font-mono">{o.progress}</span>
                  </div>
                </td>
                <td className="px-4 py-3 border-t border-border">
                  <span className={`inline-flex items-center gap-1 px-[9px] py-[3px] rounded-full text-[11px] font-bold ${
                    o.status === "Active" ? "bg-fb-green-bg text-fb-green" :
                    o.status === "Pending" ? "bg-fb-amber-bg text-fb-amber" :
                    o.status === "Done" ? "bg-fb-blue-bg text-fb-blue" :
                    "bg-fb-red-bg text-fb-red"
                  }`}>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-border">
          <span className="text-xs text-text3">Showing 1–10 of 847 orders</span>
          <div className="flex gap-1">
            ...
            <button className="w-7 h-7 rounded-md flex items-center justify-center text-xs font-semibold border border-border bg-card text-muted-foreground hover:bg-primary hover:text-primary-foreground hover:border-primary transition-all">3</button>
            <button className="w-7 h-7 rounded-md flex items-center justify-center text-xs font-semibold border border-border bg-card text-muted-foreground hover:bg-primary hover:text-primary-foreground hover:border-primary transition-all">›</button>
          </div>
        </div>
      </div>
